...
public:
    ...
    //! \throw InvalidInputException in case the input violates the precondition.
    void transfer(const SurfaceMesh& deformed_source, SurfaceMesh& deformed_target);

private:
    void compute_cotan_weights(const SurfaceMesh& mesh, EdgeProperty<double>& cotan_weights);
    void lock_vertices(const SurfaceMesh& source_before, const SurfaceMesh& source_after, VertexProperty<bool>& locked);
    void extract_deformation(const SurfaceMesh& before, const SurfaceMesh& after, FaceProperty<dmat3>& defgrad);
    void compute_target_laplacians(const FaceProperty<dmat3>& defgrad, VertexProperty<dvec3>& laplacians);
